...
public class Skeletor : MonoBehaviour {
    ...
    void Update()
    {
        ...
        if (timer > 1)
        {
            if ((Mathf.Sqrt(Mathf.Pow(transform.position.x - target.position.x, 2) + Mathf.Pow(transform.position.y - target.position.y, 2))) < 4)
            {
                playerAnimator.SetBool("Attack", true);
                
                //hitboxes[myOrientation].enabled = true;
                if ((Mathf.Sqrt(Mathf.Pow(smackBoxes[myOrientation].position.x - target.position.x, 2) + Mathf.Pow(smackBoxes[myOrientation].position.y - target.position.y, 2))) < 1.2)
                {
